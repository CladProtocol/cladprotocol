...
function DashboardOverview() {
  ...
  const stats: { key: string; label: string; value: string; icon: LucideIcon }[] = [
    ...
    { key: "payments", label: "x402 payments processed", value: String(paymentsCount), icon: ShieldCheck },
  ];
    ...
    <div className="bg-white rounded-2xl p-5 sm:p-7">
      ...
      <div className="pb-6 sm:pb-8 border-b border-gray-100">
        ...
        <h1
          ...
          style={{ fontSize: "clamp(1.6rem,4vw,2.8rem)", lineHeight: 1.08, letterSpacing: "-0.03em" }}
        >
          Your fleet, <span className="italic font-light text-gray-500">verified</span> in real time.
        </h1>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-6 sm:mt-8">
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.key} className="bg-[#F5F5F5] rounded-2xl p-5 flex flex-col gap-4 border-b-[3px] border-[#ECE81A]">
              <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center">
                <Icon size={16} className="text-gray-900" />
              </div>
              <div>
                <div className="font-medium text-gray-900" style={{ fontSize: "clamp(1.4rem,3vw,2rem)", letterSpacing: "-0.02em" }}>
                  {stat.value}
                </div>
        ...
        <div className="bg-gray-900 text-white rounded-2xl p-5 sm:p-6 flex flex-col">
          ...
          <div className="flex flex-col gap-4 flex-1">
            {activity.slice(0, 5).map((a) => (
              <div key={a.id} className="flex items-start gap-3 border-t border-white/10 pt-4 first:border-0 first:pt-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    <span className="text-[10px] uppercase tracking-wider text-[#ECE81A]">{a.tag}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="mt-5 pt-4 border-t border-white/10 flex items-center justify-between">
            ...
            <MoreLink to="/dashboard/ledger" label="View ledger" className="text-white hover:!opacity-100 hover:text-[#ECE81A]" />
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 sm:mt-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <MoreLink to="/dashboard/fleet" label={`All ${fleet.length} units`} className="text-gray-900" />
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] sm:text-[14px]">
            ...
            <tbody>
              {fleet.map((a) => (
                <tr key={a.id} className="border-b border-gray-100 last:border-0 hover:bg-[#F5F5F5] transition-colors">
                  <td className="py-3.5">
                    <Link to="/dashboard/fleet/$instanceId" params={{ instanceId: a.id }} className="block">
                  ...
                  <td className="py-3.5 text-right font-medium text-gray-900">${a.earnings.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
