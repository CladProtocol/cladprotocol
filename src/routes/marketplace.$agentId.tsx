...
export const Route = createFileRoute("/marketplace/$agentId")({
  component: AgentDetail,
  loader: ({ context, params }) =>
    context.queryClient.ensureQueryData(agentQueryOptions(params.agentId)),
  head: ({ params }) => ({
    meta: [
      { title: `${params.agentId} — Clad Marketplace` },
      { name: "description", content: "Agent listing on Clad Protocol." },
    ],
  }),
});
...
function AgentDetail() {
  ...
  const deploy = useDeployAgent({
    onDeployed: (instanceId) => {
      ...
      navigate({ to: "/dashboard/fleet/$instanceId", params: { instanceId } });
    },
  });
  // Deploying needs a connected wallet + session; prompt connect when missing.
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-[28px] sm:text-[36px] font-medium text-gray-900" style={{ letterSpacing: "-0.02em" }}>
                {agent.name}
              </h1>
              {agent.verified ? (
            ...
            <div className="mt-3 text-[13px] text-gray-600">
              ...
              <span className="font-mono text-gray-400">{agent.creatorAddress.slice(0, 6)}…{agent.creatorAddress.slice(-4)}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8">
            ...
            <div className="flex flex-wrap gap-2 mt-5">
              {agent.tags.map((t) => (
                <span key={t} className="text-[12px] text-gray-600 bg-[#F5F5F5] rounded-full px-3 py-1">
                  {t}
                </span>
              ))}
            </div>
                    ...
                    <span className="flex items-center gap-0.5">
                      {Array.from({ length: 5 }).map((_, s) => (
                        <Star
                          key={s}
                          size={12}
                          className={s < r.rating ? "fill-[#ECE81A] text-[#ECE81A]" : "text-gray-300"}
                        />
                      ))}
                    </span>
                  </div>
            ...
            <div className="bg-gray-900 text-white rounded-2xl p-6 sm:p-7">
              <div className="flex items-end justify-between">
                <div>
                  ...
                  <div className="text-[32px] font-medium leading-none mt-1">{usdExact(agent.price)}</div>
                </div>
              ...
              <p className="text-[12px] text-gray-400 leading-relaxed mt-4">
                {agent.royaltyPct}% perpetual royalty to {agent.creator}. Settles in {agent.currency} on Base via x402.
              </p>
              {deploy.isError && (
                <p className="text-[12px] text-red-400 leading-relaxed mt-3">
                  {deploy.error instanceof Error ? deploy.error.message : "Deploy failed. Please try again."}
                </p>
              )}
              {!isAuthenticated && (
              ...
              <div className="bg-white rounded-2xl p-6 sm:p-7">
                ...
                <div className="flex flex-col gap-2">
                  {instances.map((inst) => (
                    ...
                    >
                      <div className="min-w-0">
                        <div className="text-[13px] font-medium text-gray-900 truncate">{inst.name}</div>
