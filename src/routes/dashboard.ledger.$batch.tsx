...
export const Route = createFileRoute("/dashboard/ledger/$batch")({
  ...
  loader: ({ context, params }) =>
    context.queryClient.ensureQueryData(attestationQueryOptions(Number(params.batch))),
  head: ({ params }) => ({ meta: [{ title: `Batch #${params.batch} — IRONCLAD Ledger` }] }),
});
...
function AttestationDetail() {
  ...
  const steps = [
    { icon: Check, title: "Actions signed", detail: `${att.actions} actions signed by instance key`, done: true },
    { icon: Hash, title: "Batch hashed (SHA-256)", detail: "Digest computed over the action set", done: true },
    { icon: Layers, title: "Merkle root computed", detail: String(att.payload.merkleRoot ?? "—"), done: true },
    {
      icon: ShieldCheck,
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-5">
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-7">
        <Link to="/dashboard/ledger" className="inline-flex items-center gap-2 text-[13px] text-gray-600 hover:text-gray-900 transition-colors mb-6">
          <ArrowLeft size={14} /> IRONCLAD ledger
        </Link>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-[26px] sm:text-[32px] font-medium text-gray-900 font-mono" style={{ letterSpacing: "-0.02em" }}>
                #{att.batchNumber.toLocaleString()}
              </h1>
              <StatusBadge status={att.status} />
            </div>
            <div className="mt-2 text-[13px] text-gray-600">
              <Link to="/dashboard/fleet/$instanceId" params={{ instanceId: att.instanceId }} className="font-medium text-gray-900 hover:underline underline-offset-2">
                {att.instanceName}
              </Link>{" "}
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={i} className="flex gap-3 pb-6 last:pb-0 relative">
                  {i < steps.length - 1 && (
                    <span className={`absolute left-[15px] top-8 bottom-0 w-px ${s.done ? "bg-[#ECE81A]" : "bg-gray-200"}`} />
                  )}
                  <span
                  ...
                  <div className="min-w-0 pt-1">
                    ...
                    <div className="text-[12px] text-gray-500 font-mono break-all mt-0.5">{s.detail}</div>
                  </div>
                </div>
              );
            })}
        ...
        <div className="lg:col-span-2 flex flex-col gap-4 sm:gap-5">
          <div className="bg-white rounded-2xl p-5 sm:p-7">
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
              ...
              <Detail label="Window" value={`${formatTime(att.windowStart)} – ${formatTime(att.windowEnd)}`} />
          ...
          <div className="bg-white rounded-2xl p-5 sm:p-7">
            ...
            <div className="rounded-2xl bg-gray-900 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-2.5 border-b border-white/10">
                <span className="text-[11px] uppercase tracking-wider text-gray-500 font-mono">application/json</span>
...
function Detail({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      ...
      <dd className={`text-[14px] text-gray-900 break-all ${mono ? "font-mono text-[13px]" : ""}`}>{value}</dd>
    </div>
  );
}
