...
export const Route = createFileRoute("/dashboard/ledger")({
  ...
  head: () => ({ meta: [{ title: "IRONCLAD Ledger — Clad Command Center" }] }),
});
    ...
    <div className="flex flex-col gap-4 sm:gap-5">
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-7">
        ...
        <p className="mt-3 text-[14px] text-gray-600 max-w-2xl leading-relaxed">
          Actions are batched, hashed with SHA-256, and anchored to Arweave for permanent provenance. Open any batch to
          inspect its payload and verification trail.
        </p>
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-7">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] sm:text-[14px]">
            ...
            <tbody>
              {rows.map((a) => (
                <tr key={a.id} className="border-b border-gray-100 last:border-0 group hover:bg-[#F5F5F5] transition-colors">
                  <td className="py-3.5">
                    <Link to="/dashboard/ledger/$batch" params={{ batch: String(a.batchNumber) }} className="block">
                      <span className="font-medium text-gray-900 font-mono">#{a.batchNumber.toLocaleString()}</span>
                    </Link>
                  </td>
                  <td className="py-3.5 px-2 text-gray-700 hidden md:table-cell">{a.instanceName}</td>
                  <td className="py-3.5 px-2">
                    <StatusBadge status={a.status} />
                  </td>
                  <td className="py-3.5 px-2 text-gray-700 font-mono text-[12px] hidden sm:table-cell">{a.actions}</td>
                  <td className="py-3.5 px-2 text-gray-500 font-mono text-[12px] hidden lg:table-cell">{shortHash(a.sha256)}</td>
                  <td className="py-3.5 px-2 text-gray-500 hidden sm:table-cell">{formatTime(a.at)}</td>
                  <td className="py-3.5">
                    <Link to="/dashboard/ledger/$batch" params={{ batch: String(a.batchNumber) }} className="block text-gray-400 group-hover:text-gray-900 transition-colors">
