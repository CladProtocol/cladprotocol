...
export const Route = createFileRoute("/dashboard/fleet/$instanceId")({
  component: InstanceDetail,
  loader: ({ context, params }) =>
    context.queryClient.ensureQueryData(instanceQueryOptions(params.instanceId)),
  head: ({ params }) => ({ meta: [{ title: `${params.instanceId} — Fleet` }] }),
});
...
function InstanceDetail() {
  ...
  const statusMutation = useMutation({
    mutationFn: (status: InstanceStatus) => setInstanceStatus({ data: { instanceId, status } }),
    onSuccess: () => {
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-5">
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-7">
        <Link to="/dashboard/fleet" className="inline-flex items-center gap-2 text-[13px] text-gray-600 hover:text-gray-900 transition-colors mb-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-[24px] sm:text-[30px] font-medium text-gray-900" style={{ letterSpacing: "-0.02em" }}>
                  {inst.name}
                </h1>
              ...
              {agent && (
                <div className="mt-3 text-[13px] text-gray-600">
                  Running{" "}
                  <Link to="/marketplace/$agentId" params={{ agentId: agent.id }} className="font-medium text-gray-900 hover:underline underline-offset-2">
                    {agent.name}
                  </Link>{" "}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 sm:p-7">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-7">
            <div>
              <div className="text-[24px] font-medium text-gray-900">{inst.actions.toLocaleString()}</div>
              <div className="text-[12px] text-gray-500">Actions</div>
            </div>
            <div>
              <div className="text-[24px] font-medium text-gray-900">${inst.earnings.toLocaleString()}</div>
              <div className="text-[12px] text-gray-500">Earnings</div>
            </div>
            <div>
              <div className="text-[24px] font-medium text-gray-900">{timeAgo(inst.lastActivityAt)}</div>
              <div className="text-[12px] text-gray-500">Last active</div>
            </div>
            <div>
              <div className="text-[24px] font-medium text-gray-900 capitalize">{inst.kind === "physical" ? "Robotics" : "Digital"}</div>
            ...
            {inst.kind === "physical" ? (
              <>
                ...
                <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                  <div className="h-full rounded-full bg-[#ECE81A]" style={{ width: `${inst.batteryLevel ?? 0}%` }} />
                </div>
                <div className="flex items-center justify-between pt-2">
                  <span className="flex items-center gap-2 text-[13px] text-gray-400">
                    <Radio size={15} className="text-[#ECE81A]" /> ROS 2 link
                  </span>
                  <span className="text-[14px] font-medium">{inst.rosConnected ? "Connected" : "Offline"}</span>
                </div>
              </>
            ) : (
              <p className="text-[13px] text-gray-400 leading-relaxed">
                Digital agent — no hardware telemetry. Execution traces are signed and anchored to the IRONCLAD ledger
                every batch window.
              </p>
            )}
          </div>
          <div className="mt-5 pt-4 border-t border-white/10">
            <MoreLink to="/dashboard/ledger" label="View attestations" className="text-white hover:text-[#ECE81A]" />
          </div>
        </div>
      </div>
            ...
            <tbody>
              {inst.recentActions.map((a) => (
                <tr key={a.id} className="border-b border-gray-100 last:border-0">
                  ...
                  <td className="py-3.5 px-2 text-gray-500 hidden sm:table-cell">{timeAgo(a.at)}</td>
