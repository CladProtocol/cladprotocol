...
import type { NotificationPrefs } from "@/api/settings";
...
function SettingsContent() {
  ...
  const saveMutation = useMutation({
    mutationFn: (input: { displayName?: string; notifications?: NotificationPrefs }) =>
      apiPost("/api/settings", input),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["settings"] }),
  });

  const haltMutation = useMutation({
    mutationFn: () => apiPost("/api/fleet/halt"),
    onSuccess: () => {
  ...
  const notifications: { key: keyof typeof toggles; label: string; desc: string }[] = [
    {
      key: "attestations",
      label: "Attestation anchored",
      desc: "When an IRONCLAD batch lands on Arweave.",
    },
    {
      key: "settlements",
      label: "x402 settlements",
      desc: "When a payment settles for one of your units.",
    },
    ...
    <div className="flex flex-col gap-4 sm:gap-5">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <div className="text-[12px] text-gray-500">
              Operator since {formatDate(operator.joinedAt)}
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-7 border border-gray-200">
        ...
        <p className="text-[13px] text-gray-500 mb-4">
          Halting your fleet stops all units and pauses settlement.
        </p>
        <button
...
export default function SettingsPage() {
  return (
    <Suspense fallback={<div className="py-32 text-center text-gray-400">Loading settings…</div>}>
      <SettingsContent />
    </Suspense>
  );
}
