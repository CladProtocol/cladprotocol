...
const KIND_FILTERS: ("all" | PaymentKind)[] = [
  "all",
  "settlement",
  "royalty",
  "fork_fee",
...
function PaymentsContent() {
  ...
  const inflow = payments
    .filter((p) => p.kind !== "withdrawal" && p.status === "settled")
  ...
  const withdrawMutation = useMutation({
    mutationFn: (amount: number) => apiPost("/api/payments/withdraw", { amount }),
    onSuccess: () => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-2 hidden md:table-cell">
                      {p.instanceId ? (
                        <Link
                          href={`/dashboard/fleet/${p.instanceId}`}
...
export default function PaymentsPage() {
  return (
    <Suspense fallback={<div className="py-32 text-center text-gray-400">Loading payments…</div>}>
      <PaymentsContent />
    </Suspense>
  );
}
