    ...
    >
      <div className="flex items-center justify-between">
        <div className="w-11 h-11 rounded-full bg-[#F5F5F5] flex items-center justify-center transition-colors duration-500 group-hover:bg-gray-900">
          <KindIcon
            size={18}
            className="text-gray-900 transition-colors duration-500 group-hover:text-[#ECE81A]"
          />
        </div>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-3">
          <h3
            className="text-[18px] sm:text-[20px] font-medium text-gray-900"
...
function MarketplaceContent() {
  ...
  const filtered = useMemo(() => {
    ...
    return agents.filter((a) => {
      ...
      if (q && !`${a.name} ${a.description} ${a.category} ${a.id}`.toLowerCase().includes(q))
        return false;
      return true;
    });
    ...
    <>
      <section className="mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-12 pt-12 sm:pt-16 lg:pt-20 pb-8 sm:pb-10">
        ...
        >
          ...
          <span className="italic font-light text-gray-500">verified</span> and ready to deploy.
        </h1>
        <p className="mt-6 max-w-2xl text-[15px] sm:text-[17px] text-gray-600 leading-relaxed">
          Fork a vetted template, configure objectives, and ship to Base in minutes. Every listing
          is source-verified and anchored to the IRONCLAD ledger.
        </p>
      </section>
...
export default function MarketplacePage() {
  return (
    <Suspense
      fallback={<div className="py-32 text-center text-gray-400">Loading marketplace…</div>}
    >
      <MarketplaceContent />
    </Suspense>
  );
